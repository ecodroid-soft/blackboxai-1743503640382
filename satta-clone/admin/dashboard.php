...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($game && $number && $time) {
        ...
        $currentResults = file_exists($resultsFile) ? json_decode(file_get_contents($resultsFile), true) : [];
        $currentResults[$game] = [
            'name' => strtoupper($game),
            'time' => $time,
            'result' => $number,
            'status' => 'WIN',
            'date' => date('Y-m-d')
        ];
        file_put_contents($resultsFile, json_encode($currentResults, JSON_PRETTY_PRINT));
...
$historicalResults = $sheets->getHistoricalResults();

// Count today's results
$todayResults = array_filter($historicalResults, function($result) {
    return $result['date'] === date('Y-m-d');
});
$todayCount = count($todayResults);

// Count pending results (games without results for today)
$allGames = ['FARIDABAD', 'GAZIYABAD', 'GALI', 'DS'];
$todayGames = array_column($todayResults, 'game');
$pendingCount = count(array_diff($allGames, $todayGames));
?>
<!DOCTYPE html>
<html lang="en">
<head>
            ...
            <form method="POST" action="" class="space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                        ...
                        <select name="game" required class="w-full bg-gray-700 rounded-lg px-4 py-2 text-white">
                            <?php foreach ($allGames as $game): ?>
                            <option value="<?php echo strtolower($game); ?>"><?php echo $game; ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div>
